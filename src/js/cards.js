const description = "An extended description of the food that is unique to the particular item."
...
const foods = [
  ...
  ["Ratatouille", description, 'french'],
  ["Baguette", description, 'french']
]
...
function renderCards(filteredFoods) {
  ...
  if (filteredFoods.length === 0) { // if there are no foods when searching
...
searchInput.addEventListener('input', () => {
  const query = searchInput.value.toLowerCase().trim() // user input

  filteredFoods = currentFoods.filter(food => 
    food[0].toLowerCase().includes(query) // checks if food name includes the user search input
  )
...
// message to show if user input is wrong or when the form is complete
...
let email_check =  /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/ // email validation
const red = 'rgb(249, 5, 5)'  // underline color
const green = 'rgba(23, 230, 47, 1)'
...
submitButton.addEventListener('click', (e) => {
  ...
  if ((usernameInput.value.trim() === '' || usernameInput.value.trim() == null) &
    ...
    (messageInput.value.trim() === '' || messageInput.value.trim() == null)) {
        ...
        // adds underlines and gives color 
  ...
  // if incorret email and empty username
  ...
  // if incorrect email and empty message
  else if (!emailInput.value.trim().match(email_check) &
    (messageInput.value.trim() === '' || messageInput.value.trim() == null)) {
  ...
  // all inputs filled and correct email format
  else {
    warningMessage.innerHTML =  `<h2 style="color: ${green}">Message succesfully sent</h2>`
    // removes user input from boxes
    usernameInput.value = ''
    emailInput.value = ''
    messageInput.value = ''
